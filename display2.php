...
<body>
 ...
 <h2 align="center">Displaying the LAN Request Forms(Level 2)</h2>
 ...
 <th width="4%">End Point Type with OS details (server/workstation/Desktop)</th>
 <th width="13%">End Point changed with other Network.If yes Please format the End point</th>
 ...
 <td width='20%'><a href='display2.php?
action=approve&name=$result[name]&designation=$result[designation]&a=$result[office]&b=$result[sinvt]&c
=$result[oinvt]&d=$result[endpt]&e=$result[endpn]&f=$result[epown]&g=$result[location]&h=$result[system]&i=$result[rqend]&remarks=$result[remarks]'>Approve</a>                                  
 <a href='display2.php?action=delete&name=$result[name]&designation=$result[designation]&a=$result[office]&b=$result[sinvt]&c
...
if (isset($_GET['action']) && $_GET['action'] == 'approve') {
 echo "<h1>your application is approved at level 2</h1>";
...
$query = "INSERT INTO form3 VALUES('$name','$dsg','$of','$st','$offi','$endpt','$endptch','$endptow','$locat','$macadd','$admin','$remarks')";
$data=mysqli_query($conn,$query);
if($data){
 echo "data inserted";
}
...
$query = "INSERT INTO user2 VALUES('$nm','$pw')";
$data=mysqli_query($conn,$query);
